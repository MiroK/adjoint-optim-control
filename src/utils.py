...
def read_vtu_point_data(vtu, nvertices, ncells):
    ...
    values = np.array(list(map(float, filter(bool, data.text.split(' ')))))
...
def read_vtu_mesh(path, cell_type):
    ...
    point_data = np.array(list(map(float, filter(bool, point_data.text.split(' ')))))
    point_data = point_data.reshape((-1, 3))[:, :gdim]

    # Parse cells
    cell_data = next(iter(cells))
    cell_data = np.array(list(map(int, filter(bool, cell_data.text.split(' ')))))
